...
# =========================================================
# بکاپ و بازگردانی دیتابیس - فقط پیوی مالک اصلی
# =========================================================
...
def create_database_backup():
    with db_lock:
        ...
        backup_path = "/data/casino_meowi_backup.db"
...
@bot.message_handler(
    ...
    func=lambda m: (
        m.chat.type == "private"
        and m.from_user.id == MAIN_OWNER
    )
)
def backup_database(m):
    try:
        ...
        with open(backup_path, "rb") as backup_file:
            bot.send_document(
                m.chat.id,
                backup_file,
                caption="💾 بکاپ دیتابیس Casino Meowi Cop 👮‍♂️\n\nاین فایل را در جای امن نگه دار."
            )
...
def restore_database_command(m):
    pending_restore.add(m.from_user.id)

    bot.send_message(
        m.chat.id,
        "♻️ فایل بکاپ دیتابیس را همینجا برای من ارسال کن.\n\n"
        "فقط فایل بکاپی که قبلاً از همین ربات گرفته‌ای را بفرست."
    )
...
@bot.message_handler(
    ...
    func=lambda m: (
        ...
        and m.from_user.id == MAIN_OWNER
        and (
            m.from_user.id in pending_restore
            or (
                m.caption
                and m.caption.strip().startswith("/restore")
            )
        )
    )
)
def restore_database_file(m):
    temp_path = "/data/casino_meowi_restore_temp.db"

    try:
        file_info = bot.get_file(m.document.file_id)
        downloaded_file = bot.download_file(file_info.file_path)

        with open(temp_path, "wb") as restore_file:
        ...
        pending_restore.discard(m.from_user.id)

        bot.send_message(
            m.chat.id,
            "✅ دیتابیس با موفقیت بازگردانی شد.\n\n"
            "اطلاعات قبلی دیتابیس جایگزین شد."
        )
